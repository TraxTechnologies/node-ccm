...
const LOOPBACK_IF_UP_REG = /lo0: flags=.*\WUP\W.*$/;
const LOOPBACK_ALIAS_REG = /inet (127\.\d{1,3}\.\d{1,3}\.\d{1,3}) netmask/;
...
export default class Cluster {
  ...
  verifyLoopbackAliases() {
    ...
    ifconfig.stdout.pipe(split()).on('data', (line) => {
      if (line.match(LOOPBACK_IF_UP_REG)) {
        loopbackUp = true;
      } else if (line.match(LOOPBACK_ALIAS_REG)) {
        const [, loopbackAddress] = LOOPBACK_ALIAS_REG.exec(line);
        configuredAliases.push(loopbackAddress);
      }
    });
      ...
      ifconfig.once('exit', () => {
        ...
        if (!loopbackUp || !_.some(missingAliases)) {
          if (configureLoopbackAliases) {
            this.log('info', 'Loopback interface down. Bringing it back up will require sudo privileges.');
            const loopback = spawn('sudo', ['ifconfig', 'lo0', 'up']);
          ...
          if (configureLoopbackAliases) {
            this.log('info', `Loopback address not configured for [${missingAliases.join(', ')}]. Configuring it will require sudo privileges.`);
            _.reduce(
              missingAliases,
              (p, address) =>
                p.then(() => {
                  const loopback = spawn('sudo', ['ifconfig', 'lo0', 'alias', address, 'up']);
