...
const FAKE_ERROR_REG = /ccm INFO Downloading http/;
    ...
    this.process.stderr
      ...
      .on('data', (line) => {
        if (line.trim() && (!skipLogs || verbose)) {
          // ccm pipes some info messages to stderr instead of stdout. These should not be error
          // messages
          if (line.match(FAKE_ERROR_REG)) {
            this.log('info', line);
          } else {
            this.log('error', line);
          }
