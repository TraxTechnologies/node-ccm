    ...
    return addresses.reduce((address) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.log(
          ...
          `Address ${address}:${
            defaultPort
          } is not available. Cassandra may already be running externally.`
        );
  ...
  verifyClusterDir() {
    ...
    return fs.pathExists(resolvedPath).then((exists) => {
      if (exists) {
        ...
        const msg = `Cluster config directory already exists: ${
          resolvedPath
        }.\n\tUse the \`purge\` option to remove it automatically or remove it manually with \`ccm switch ${
          clusterName
        }; ccm remove;\``;
  ...
  configurePortsAndAddresses() {
    ...
    return Promise.all(_.times(nodes).map((number) => {
      ...
      const addressUpdate = fs.readFile(yamlPath).then((data) => {
        ...
        return fs.writeFile(yamlPath, newContent).then(() => [oldAddress, address]);
      });
      ...
      const jmxPortUpdate = fs.readFile(shPath).then((data) => {
        ...
        return fs.writeFile(shPath, newContent).then(() => [oldPort, port]);
      });
      return Promise.all([addressUpdate, jmxPortUpdate]).then(([address, port]) => ({
        node: nodeName,
        address,
        port
      }));
      ...
      const clusterConfP = fs.readFile(clusterConfPath).then((data) => {
      ...
      const nodeConfP = _.map(
        ...
        ({ node, address: [oldAddress, newAddress], port: [oldPort, newPort] }) => {
          ...
          return fs.readFile(nodeConfPath).then(data =>
            fs.writeFile(
              nodeConfPath,
              data
                .toString()
